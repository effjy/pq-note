# Makefile for PQ-Note - GTK3 post-quantum encrypted notes.
#
#   make                build the ./pqnote binary
...
CFLAGS  += -DPQNOTE_VERSION=\"$(VERSION)\" -DKYBER_K=4 -Isrc/kyber \
...
SRC = src/main.c src/vault.c src/secure_buffer.c \
...
install: $(BIN)
	...
	install -m 0644 data/pqnote.svg $(DESTDIR)$(ICONDIR)/pqnote.svg
	for s in $(ICON_SIZES); do \
	    install -d $(DESTDIR)$(ICONBASE)/$${s}x$${s}/apps; \
	    install -m 0644 data/pqnote-$${s}.png \
	        $(DESTDIR)$(ICONBASE)/$${s}x$${s}/apps/pqnote.png; \
	done
	install -d $(DESTDIR)$(APPDIR)
	install -m 0644 data/pqnote.desktop $(DESTDIR)$(APPDIR)/pqnote.desktop
	...
	@echo "PQ-Note $(VERSION) installed to $(BINDIR)/$(BIN)"

uninstall:
	rm -f $(DESTDIR)$(BINDIR)/$(BIN)
	rm -f $(DESTDIR)$(ICONDIR)/pqnote.svg
	for s in $(ICON_SIZES); do \
	    rm -f $(DESTDIR)$(ICONBASE)/$${s}x$${s}/apps/pqnote.png; \
	done
	rm -f $(DESTDIR)$(APPDIR)/pqnote.desktop
	-update-desktop-database $(DESTDIR)$(APPDIR) 2>/dev/null || true
	-gtk-update-icon-cache -f -t $(DESTDIR)$(DATADIR)/icons/hicolor 2>/dev/null || true
	@echo "PQ-Note uninstalled"
