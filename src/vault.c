/*
 * vault.c - PQ-Note vault model + in-memory authenticated encryption.
 *
 * Vault file format (all integers little-endian):
 *
 *   off  size  field
 *   ---  ----  -------------------------------------------------------------
 *   0    8     magic  "PQNOTE\0\0"
 ...
 *   "PQNV" magic (4) | uint32 version | uint32 n_entries |
...
#define WRAP_AD     ((const unsigned char *)"PQNOTE-HYBRID-WRAP")
...
int vault_save(vault_t *v, const char *path, const char *password,
    ...
    if (snprintf(tmp, sizeof(tmp), "%s.pqnote-tmp", path) >= (int)sizeof(tmp)) {
...
int vault_load(const char *path, const char *password, vault_t **out,
    ...
    *out = NULL;
    ...
    if (fread(hdr, 1, HEADER_LEN, in) != HEADER_LEN ||
        ...
        seterr(err, errlen, "Not a PQ-Note vault (bad magic)."); goto done;
    }
